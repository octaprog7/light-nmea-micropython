...
_NAV_PACKS = (
    ...
    # 25. БИТЫЙ ЧИСЛОВОЙ: RMC скорость = 'xyz' (CRC валидна!) -> speed=None, курс сохраняется
    b"$GNRMC,120000,A,4807.038,N,01131.000,E,xyz,084.4,220626,,,A*30\r\n",
    # 26. БИТЫЙ ЧИСЛОВОЙ: RMC курс = 'abc' (CRC валидна!) -> course=None, скорость сохраняется
    b"$GNRMC,120000,A,4807.038,N,01131.000,E,022.4,abc,220626,,,A*27\r\n",
    # 27. БИТЫЙ ЧИСЛОВОЙ: GGA HDOP = 'abc' (CRC валидна!) -> hdop=None, остальное сохраняется
    b"$GNGGA,120000.00,5545.1234,N,03737.5678,E,1,12,abc,156.3,M,14.2,M,,*33\r\n",
    # 28. БИТЫЙ ЧИСЛОВОЙ: GGA спутники = 'xx' (CRC валидна!) -> satellites=0
    b"$GNGGA,120000.00,5545.1234,N,03737.5678,E,1,xx,0.8,156.3,M,14.2,M,,*76\r\n",
    # 29. БИТЫЙ ЧИСЛОВОЙ: GGA высота = '15x.3' (CRC валидна!) -> altitude=None
    b"$GNGGA,120000.00,5545.1234,N,03737.5678,E,1,12,0.8,15x.3,M,14.2,M,,*3B\r\n",
    # 30. БИТЫЙ ЧИСЛОВОЙ: VTG скорость км/ч = '4.72x' (CRC валидна!) -> берётся knots
    b"$GPVTG,084.4,T,081.3,M,022.4,N,4.72x,K,A*6C\r\n",
    # 31. БИТЫЙ ЧИСЛОВОЙ: VTG курс = '08x.4' (CRC валидна!) -> course=None, скорость сохраняется
    b"$GPVTG,08x.4,T,081.3,M,022.4,N,041.5,K,A*69\r\n",
)
